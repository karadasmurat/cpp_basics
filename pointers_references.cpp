...
An object that holds an address value is called a pointer. For example, the type needed to hold the address of an int is called a “pointer to int” or an “int pointer” and the notation is int*
  ...
  (sometimes called code storage or text storage) and for the global variables you define (called static storage).
  It also sets aside some memory to be used when you call functions, and they need space for their arguments and local variables (that’s called stack storage or automatic storage).
  The rest of the computer’s memory is potentially available for other uses; it is “free”
...
If it created several objects (an array), it returns a pointer to the first of those objects.
(A pointer does not know how many elements it points to, the value is the address of the first byte of the memory.)
  ...
  A pointer variable has its own identity : a distinct, visible memory address that can be taken with the unary &operator
  and a certain amount of space that can be measured with the sizeof operator.
  ...
  */
  ...
  /* Access through pointers:
    ...
    Advice: Initialize all Pointers. Uninitialized pointers are a common source of run-time errors.
    What happens when we use an uninitialized pointer is undefined.
    Using an uninitialized pointer almost always results in a run-time crash.
    Debugging the resulting crashes can be surprisingly hard - There is no way to distinguish a valid address from an invalid one formed from the bits that happen to be in the memory in which the pointer was allocated. (garbage!)
...
*/
  ...
  // *notInitializedPtr = 7.0; // ERR: dereferencing a pointer without initializing it is asking for trouble.

  *ptr += 5; // a is now 10, When we assign to *ptr we are assigning to the object to which ptr points.
...
*/
  ...
  If we look at the definition of pd, we find that the answers are yes and no, respectively.
  However, the compiler doesn’t know that; it does not keep track of pointer values.
  Our code will simply access memory as if we had allocated enough memory.
  It will even access pd[–3]. We have no idea what the memory locations marked pd[–3] and pd[4] are used for.
  However, we do know that they weren’t meant to be used as part of our array of three doubles pointed to by pd.
  Most likely, they are parts of other objects and we just scribbled all over those. That’s not a good idea.
  In fact, it is typically a disastrously poor idea: “disastrous” as in “My program crashes mysteriously” or “My program gives wrong output.”
  Out-of-range access is particularly nasty because apparently unrelated parts of a program are affected.
  An out-of-range read gives us a “random” value that may depend on some completely unrelated computation:
*/
  ...
  Since a computer’s memory is limited, it is usually a good idea to return memory to the free store once we are finished using it.
  That way, the free store can reuse that memory for a new allocation.
  For large programs and for long-running programs such freeing of memory for reuse is essential.
  ...
  */
  ...
  // We do not know looking at caller syntax if parameter is int reference or int !! We have to look method signature.
  increment_ref(x);
  cout << "x: " << x << endl; // 12
  ...
  There are several ways to obtain a null pointer, and the most direct approach is to initialize the pointer using the literal nullptr.
  ...
  There is no way to distinguish a valid address from an invalid one formed from the bits that happen to be in the memory in which the pointer was allocated. (garbage!)
...
void pointerToAPointer()
{
  ...
  int **ppi = &pi; // ppi points to a pointer to an int (2 kere sekerek objeye gidersin)
...
void referenceBasics()
{
  ...
  // Because references are not objects, they don’t have addresses, &xRef returns the address of the referenced object:
  cout << "x      : " << x << endl;
  cout << "xRef   : " << xRef << endl;
  ...
  */
  ...
  // A pointer needs to be dereferenced  (*) to access the memory location it points to, whereas a reference can be used directly.
  *xptr += 1;
  ...
  // call like regular variable: we do not know looking at caller syntax if parameter is int reference or int !! We have to look method signature.
  increment_ref(x);
